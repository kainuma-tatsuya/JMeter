    ...
    private UrlEncodedFormEntity createUrlEncodedFormEntity(final String contentEncoding) throws UnsupportedEncodingException {
        ...
        while (args.hasNext()) {
            ...
            if (!arg.isAlwaysEncoded()) {
                ...
                //XXX parameterName = URLDecoder.decode(parameterName, urlContentEncoding);
                //XXX parameterValue = URLDecoder.decode(parameterValue, urlContentEncoding);
		URLCodec codec = new URLCodec(urlContentEncoding); //XXX
		try {  //XXX
		    parameterName = codec.decode(parameterName); //XXX
		    parameterValue = codec.decode(parameterValue);  //XXX
		} catch (Exception e) { e.printStackTrace(); } //XXX
            }
