    ...
    public HTTPArgument(String name, String value, boolean alreadyEncoded, String contentEncoding) {
        ...
        if (alreadyEncoded) {
            try {
                ...
                //XXX name = URLDecoder.decode(name, EncoderCache.URL_ARGUMENT_ENCODING);
                URLCodec codec = new URLCodec(EncoderCache.URL_ARGUMENT_ENCODING); //XXX
                name = codec.decode(name); //XXX
                ...
                //XXX value = URLDecoder.decode(value, contentEncoding);
                codec = new URLCodec(contentEncoding); //XXX
                value = codec.decode(value); //XXX
            //XXX } catch (UnsupportedEncodingException e) {
            } catch (DecoderException e) { //XXX
